...
class MusicController extends Controller
{
    ...
    public function saveStep2(Request $request) {
        ...
        $validatedData = $request->validate([
            //dimensions:min_width=3000,min_height=3000
            //mimes:png,jpeg,jpg,webp|max:2048
            'thumbnail' => 'required',
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'trackFile' => 'required|array|max:' . $count,
            'trackFile.*' => 'file|max:20480', // This ensures each file in the array does not exceed 20480 KB (20 MB)
        ]);
        ...
        foreach ($request->file('trackFile') as $track) {
            $path = $track->storeAs('music/'.$user_id.'/'. $trackcode.'/tracks',$track->getClientOriginalName(), 'public');  
            $trackPaths[] = $path;
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->hasFile('trackFile')){
           ...
           foreach( $old_tracks_paths as $track){
                if (Storage::disk('public')->exists($track)) {
