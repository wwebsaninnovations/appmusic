    ...
    <div class="row">
        <div class="col-md-2 tabs-item">
            ...
            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                ...
                <button class="nav-link {{($level=='summery')? ' active':''}}" id="v-pills-summery-tab" data-bs-toggle="pill" data-bs-target="#v-pills-summery" type="button" role="tab" aria-controls="v-pills-summery" aria-selected="false">Summery</button>
            </div>
        </div>
        <div class="col-md-10">
                    ...
                    <div class="container mt-5">
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <p><strong>Remixer:</strong> {{ $release->featuring_artist_basic ?? '' }}</p>
                                <p><strong>Producer:</strong> {{ $release->producer_artist ?? '' }}</p>
                                    ...
                                    <div class="col-6">
                                        <div class="tab-content" id="v-pills-tabContent">
                                            ...
                                            <div class="tab-pane fade {{ ($index == 0) ? 'show active' : '' }}" id="v-pills-track{{ $index }}" role="tabpanel" aria-labelledby="v-pills-track{{ $index }}-tab">
                                                ...
                                                <p><strong>Version:</strong> {{ old('track_version.'.$index) }}</p>
                                                <p><strong>Lyrics Language:</strong> {{ old('lyrics_language.'.$index) }}</p>
                                                <p><strong>Explicit Content:</strong> {{ old('explicit_content.'.$index) }}</p>

                                                <h5>Contributor</h5>
                                                <p><strong>Primary Artist:</strong> {{ old('primary_artist.'.$index) }}</p>
                                                <p><strong>Featuring Artist:</strong> {{ old('featuring_artist.'.$index) }}</p>
                                                <p><strong>Remixer:</strong> {{ old('track_remixer.'.$index) }}</p>
                                                <p><strong>Song Writer:</strong> {{ old('song_writer.'.$index) }}</p>
                                                <p><strong>Producer:</strong> {{ old('track_producer.'.$index) }}</p>

                                                <h5>Composer</h5>
                                                <p><strong>Composer Name:</strong> {{ old('composer_name.'.$index) }}</p>
                                                <p><strong>Label Name:</strong> {{ old('label_name.'.$index) }}</p>
                                                <p><strong>ISRC:</strong> {{ old('isrc.'.$index) }}</p>

                                                <h5>Master Right</h5>
                                                <p><strong>Primary Performers:</strong> {{ old('primary_performers.'.$index) ? 'Yes' : 'No' }}</p>
                                                <p><strong>Publisher Name:</strong> {{ old('pname.'.$index) }}</p>
                                                <p><strong>C Name:</strong> {{ old('cname.'.$index) }}</p>
                                                <p><strong>Ownership for the sound recording:</strong> {{ old('ownership_for_sound_rec.'.$index) }}</p>
                                                <p><strong>Country of Recording:</strong> {{ old('country_of_rec.'.$index) }}</p>
                                                <p><strong>Nationality of Original Copyright Owner:</strong> {{ old('nationality.'.$index) }}</p>
                                            </div>
                                            @endforeach
                                        </div>
                                    </div>
